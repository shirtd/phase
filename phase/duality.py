...
from phase.topology.util import *
...
class VoronoiDual:
    def __init__(self, P, F, R):
        ...
        self.tets = sorted(F.K[3], key=lambda t: t.data['alpha'])
        self.imap = {t : i for i,t in enumerate(self.tets)}
        self.Q = np.vstack([tet_circumcenter(P[list(t)]) for t in self.tets])
        self.emap = {f : [self.imap[t] for t in f.cofaces] for f in F.K[2] if len(f.cofaces) == 2}# if not F.index(f) in R}
        self.nbrs = {i : set() for i,_ in enumerate(self.tets)}
        for u,v in self.emap.values():
            self.nbrs[u].add(v)
            self.nbrs[v].add(u)
        self.fmap = {e : to_path({v for f in e.cofaces for v in self.emap[f]}, self.nbrs) for e in F.K[1] if all(f in self.emap for f in e.cofaces)}# if not F.index(e) in R}
        self.cmap = {v : [self.fmap[e] for e in v.cofaces] for v in F.K[0] if all(e in self.fmap for e in v.cofaces)}# if not F.index(v) in R}
    ...
    def tet_chain_dual(self, C, ext=False):
        ...
        edges = {e for f in faces for e in self.F.K(f).faces}
